...
const SignUp = () => {
  ...
  return (
    <RouteWrapper>
      ...
      {!isLoading && (
        <Wrapper>
          <Heading style={{marginTop:'-20px'}}>به وب سایت بابا کد خوش امدید</Heading>
          <Paragraph style={{ fontSize: '1.2rem' }}>  <p style={{ fontSize: '1.2rem' }}>
           مرجع آموزش های رایگان برنامه نویسی آموخته های خود را با دیگران به اشتراک بگذارید </p><br /> ثبت نام با اکانت های گوگل و گیت هاب امکان پذیر هست
          ...
          <form onSubmit={handleSubmit}>
            <InputContainer>
              <Label style={{ textAlign: 'right', fontWeight: 'bold' }} htmlFor='name'>اسم :</Label>
              <Input
                ref={nameRef}
                autoComplete='off'
                required
              ...
              />
              {!validName && name && <Error>اسم معتبر نیست . نامتان را به انگلیسی باید وارد کنید . نام نمیتواند شامل عدد باشد</Error>}
            </InputContainer>

            <InputContainer>
              <Label style={{ textAlign: 'right', fontWeight: 'bold' }} htmlFor='username'>نام کاربری :</Label>
              <Input
                autoComplete='off'
                required
                id='username'
              ...
              />
              {!validUser && user && <Error>نام کاربری معتبر نیست . باید حداقل 4 حرف باشد </Error>}
            </InputContainer>

            <InputContainer>
              <Label style={{ textAlign: 'right', fontWeight: 'bold' }} htmlFor='email'>ایمیل :</Label>
              <Input required id='email' value={email} onChange={e => setEmail(e.target.value)} />
              {!validEmail && email && <Error>ایمیل معتبر نیست</Error>}
            </InputContainer>
            <InputContainer>
              <Label style={{ textAlign: 'right', fontWeight:'bold' }} htmlFor='password'>پسورد :</Label>
              ...
              />
              {!validPwd && pwd && <Error>پسورد معتبر نیست از حروف و بزرگ و کوچک استفاده کنید و حداقل 8 کاراکتر باشد . حداقل بایستی یک کاراکتر مانند @ یا % و غیره باشد</Error>}
            </InputContainer>
            ...
            {isError && <Error>یا ایمیل یا نام کاربری در سایت وجود دارد نام کاربری و ایمیل خود را بررسی کنید شاید قبلا ثبت نام کرده اید</Error>}

            <Submit style={{fontWeight:'bold'}}>ثبت نام</Submit>
          </form>
        </Wrapper>
      )}
    </RouteWrapper>
